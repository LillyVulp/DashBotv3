...
bot.on("guildCreate", async function(guild) {
	...
	try {
		if(guild.members.find(val => val.id == process.env.tatsuID)) {
			await sleep(5000);
			guild.members.find(val => val.id == process.env.tatsuID).lastMessage.channel.send("Hi, Tatsu :3").then(function(response) {
...
function exec(message) {
	if(message.author.id == ownerID) {
		try {
			...
			message.channel.send("An error occured - " + e.toString());
		}
	}
}
...
function chatbot(message) {
	...
	input.shift();
	input = input.join(" ");
	request("https://www.pandorabots.com/pandora/talk-xml?botid=" + process.env.pandoraID + "&input=" + encodeURIComponent(input) + "&custid=" + message.author.id + "000055", function(err, response, body) {
		if(err) {
			console.log(err);
		...
		if(err) {console.log(err); message.channel.send("Memory machine broke, please try again later");}
			...
			database.query("SELECT money FROM users WHERE user_id=" + message.author.id + "::text", function (err, response) {
				...
				if(err) {console.log(err); message.channel.send("Memory machine broke, please try again later");}
					...
					message.channel.send("You have earned $50. You now have $" + response.rows[0].money + ".");
				}
			});
		}
	});
	...
	database.query("SELECT money FROM users WHERE user_id=" + message.author.id + "::text", function (err, response) {
		if(err) {console.log(err); message.channel.send("Memory machine broke, please try again later");}
			...
			if(typeof response.rows[0] == "undefined") {
				...
				message.channel.send("You have $" + response.rows[0].money + ".");
			}
		}
	});
};
...
function insult(message) {
	if(message.content.indexOf(" ") == -1) {
		message.channel.send("I'm just gonna assume you want me to insult you... after all... why wouldn't I x333");
	}
...
function bap (message) {
	if(message.content.indexOf(" ") == -1) {
		...
		message.channel.send("By the power vested in me, I BAP THEE, " + message.content.split(" ")[1] + "!\n *bap* :french_bread:");
	}
};
